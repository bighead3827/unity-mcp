...
def _read_existing_examples(path: Path) -> str:
    ...
    match = re.search(
        rf"{re.escape(EXAMPLES_OPEN)}(.*?){re.escape(EXAMPLES_CLOSE)}",
        text,
        re.DOTALL,
    )
    if not match:
    ...
    if not captured.strip():
        return EXAMPLES_PLACEHOLDER
    return f"{EXAMPLES_OPEN}{captured}{EXAMPLES_CLOSE}\n"
...
def generate(
    ...
    # Per-tool pages + per-group landing.
    for group, group_tools in sorted(tools_by_group.items()):
        ...
        for tool in sorted(group_tools, key=lambda t: t["name"]):
            ...
            existing_examples = _read_existing_examples(page_path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        try:
            ...
            generate(tmp_tools, tmp_resources)
            if keep:
